...
var modals = (function(){
     ...
     */
    var tpl = {
	startGame :	'<div class="modals" id="inviteModal" ><h1>Four in a row</h1> <hr /><h2></h2> <p> <button class="delayed blue" id="invite" >Start</button> <button class="delayed blue" id="cancel" >Quit</button> </p></div>',
	giveUp :  	'<div class="modals" id="giveUpModal" ><h1>Give up already?</h1> <hr /> <div class="modal-button" id="continue">Still can do it</div> <div class="modal-button" id="yes" >Yes</div>',
	invite :  	'<div class="modals" id="inviteModal" ><h1>Battle Fleets</h1>  <hr /><h2>Invite to Play</h2><hr /> <p> <button class="delayed blue" id="invite" >Invite</button> <button class="delayed blue" id="cancel" >Cancel</button> </p></div>',
	invited :  	'<div class="modals" id="inviteModal" ><h1>Battle Fleets</h1> <hr /><h2>Invitation to Play</h2><hr /> <p> <button class="delayed green" id="play" >Play</button> <button class="delayed red" id="quit" >Quit</button></p></div>',
	waiting : 	'<div class="modals moveSentBox" id="waitModal"><h1>Not your turn yet</h1> <p></p></div>',
	capture : 	'div class="modals moveSentBox" id="waitModal"><h1>You must capture your opponent`s piece.</h1> <p></p></div>',
	sent : 		'<div class="modals" id="sentModal"> <h1>Invitation Sent</h1> <p>Invitation has been sent!</p> </div>',
	win : 		'<div class="modals" id="winModal"><h1>Winner</h1> <p>Congratulations you beat your opponent.<br></p></div>',
	win_by_friend_quit : '<div class="modals" id="winModal"><h1>Winner</h1> <p>Your friend has left the game.<br></p></div>',
	lose : 		'<div class="modals" id="loseModal"><h1>Loser</h1> <p>Commiserations your opponent won this round.<br></p></div>',
	move : 		'<div class="modals" id="moveModal"> <h1>Its your turn!</h1> <p>Make your move!</p> </div>',
	yourturn : 	'<div class="modals" id="yourturnModal">Your turn!</div>',
	reject : 	'<div class="modals" id="moveModal"> <h1>Rejected</h1> <p>Your request was Rejected</p> </div>',
	moveSent : 	'<div class="modals moveSentBox" id="yourturnModal"><h1>Move sent!</h1> <p></p>\ </div>',
	notice : 	'<div class="modals" id="attackMove"> <h1>Notice</h1> <p><br /><!--message--> <br />&nbsp </p> </div>',
	loading : 	'<div class="modals"> <img src="img/ajax-loader.gif"> </div>',
    };
	...
	var replay = '<div class="modals"> <div class="'replayModalBox"><h2>Showing</h2><h1>{{line1}}`s Move</h1></div> <img class="replayModalImg" src="{{gameimg}}"> </div>'
...
<img class='replayModalImg' src='{{gameimg}}'>\
    ...
    var showCaptureModal = function(){

	killModal();

	masterModal = picoModal({content: tpl.capture, width:'95%' ,  closeButton:false, overlayClose:false, overlayStyles: function ( styles ) { styles.opacity = 0; }, modalStyles: function ( styles ) { styles.visibility = "hidden", styles.padding="15px", styles.opacity="0"}} ).afterShow(function(modal){
    ...
    var showYesNoDialog = function(game, title, yesString, noString, callback){
	killModal();
...
<div class="modal-button" id="yes" >{{yesString}}</div>\
	...
	masterModal = picoModal({content: tpl.giveUp, width:'95%' ,  closeButton:false, overlayClose:false, modalStyles: function ( styles ) { styles.visibility = "hidden", styles.padding="15px", styles.opacity="0"}} ).afterShow(function(){
		...
		nativeBridge.sendMove(env.data);
		killModal();
		nativeBridge.closeView();		
	    });
	}).show();
