...
var modals = (function(){
	...
	waiting : '<div class="modals moveSentBox" id="waitModal"><h1>Not your turn yet</h1>\
              <p></p></div>\
',
	inCheck : '<div class="modals moveSentBox" id="waitModal"><h1>You are in check</h1>\
<p></p></div>\
',
...
<img src="img/ajax-loader.gif"> </div>',
	check :  '<div class="modals  moveSentBox" id="waitModal" ><h1>{{color}} in check</h1> \
</div>',
	checkmate :  '<div class="modals  moveSentBox" id="waitModal" ><h1>{{color}} checkmate</h1> \
</div>',
    ...
    // ..
    var showYouAreInCheckModal = function(){

	killModal();

	masterModal = picoModal({content: tpl.inCheck, width:'95%' ,  closeButton:false, overlayClose:false, overlayStyles: function ( styles ) { styles.opacity = 0; }, modalStyles: function ( styles ) { styles.visibility = "hidden", styles.padding="15px", styles.opacity="0"}} ).afterShow(function(modal){
    ...
    // 	killModal();
    // 	masterModal = picoModal({content: tpl.yourturn, width:'70%', closeButton:false, overlayClose:false });
    // 	masterModal.show();
    // 	setTimeout(function() {
    // 	    killModal();
    // 	}, 1000, typeOfMove);
...
<img class='replayModalImg' src='{{gameimg}}'>\
    ...
    var showCheckModal = function(color){
	killModal();

	var html = tpl.check.split('{{color}}').join(color);
	
	masterModal = picoModal({content: html, width:'95%' ,  closeButton:false, overlayClose:false, overlayStyles: function ( styles ) { styles.opacity = 0; }, modalStyles: function ( styles ) { styles.visibility = "hidden", styles.padding="15px", styles.opacity="0"}} ).afterShow(function(modal){
    ...
    var showCheckMateModal = function(color){
	killModal();

	var html = tpl.checkmate.split('{{color}}').join(color);
...
<img class='replayModalImg' src='{{gameimg}}'>\
	...
	if(env.data.profile.yo.img == ""){
	    ...
	    replay = replay.split('{{gameimg}}').join(env.data.profile.yo.img);	    
	}	
	replay = replay.split('{{line1}}').join(decodeURIComponent(env.data.profile.yo.name));
	...
	showYouAreInCheckModal : showYouAreInCheckModal,
